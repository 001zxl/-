...
- For Douyin's Pet Life category, use at least three real-photography main images at 600 x 600 px or larger and prefer 1:1. Put the complete physical front view first, then use real angles, outer packaging, and readable label evidence; domestic goods need the complete Chinese label and imports need the complete simplified-Chinese label. Keep test reports, formulas, feeding or dosage instructions, cautions, risk notices, live-animal measurements, and after-sales terms as verified detail-page evidence rather than AI-generated decoration, and recheck the exact pet subcategory before upload.
...
- TikTok's September 2026 Carousel Ads overview lists Traffic, Web Conversions, App promotion, Lead generation, Product sales–Catalogue, and Brand auction reach among supported ad types, with automatic or TikTok-only placement. Standard Carousel can run as Spark or Non-Spark; VSA Carousel is Non-Spark only. Record the selected objective, identity mode, placement, and per-card link behavior before repurposing PDP or social-feed assets. Confirm live account/market eligibility rather than assuming every format is available in every campaign.
...
- TikTok's **Collage Carousel** is a separate automatic enhancement for upgraded Smart+ Catalog campaigns using the Sales objective. It can place one main catalog image plus three smaller supporting product images on the same scrollable screen, with each image linking to its product detail page. Treat the four-image composition as a placement preview requirement rather than a license to bake a collage into each source asset; keep every catalog image independently clear, correctly linked, and truthful to its SKU.
